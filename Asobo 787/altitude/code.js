...
const setAltitude = async (targetAltitude, retry) => {
  ...
  const originalAltitudeIncrement = this.$api.variables.get("L:XMLVAR_Autopilot_Altitude_Increment", "number");

  if (difference < 0) {
    const {normalStep, extraStep} = getStepCount(-difference / 100);

    this.$api.variables.set("L:XMLVAR_Autopilot_Altitude_Increment", "number", 1000);
    for (let i = 0; i < normalStep; i++) {
      await timeout(50);
      this.$api.variables.set("B:AUTOPILOT_Altitude_Inc", "number", 1);
    }

    this.$api.variables.set("L:XMLVAR_Autopilot_Altitude_Increment", "number", 100);
  ...
  this.$api.variables.set("L:XMLVAR_Autopilot_Altitude_Increment", "number", originalAltitudeIncrement);
